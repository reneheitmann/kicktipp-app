-- Saisonvergleich soll neben dem Gesamtsaldo (0063) wahlweise auch die
-- reinen Gewinne vergleichen können ("wer hat am meisten gewonnen", ohne
-- Einsatz/Zahlungen gegenzurechnen) - dieselbe Definition wie "Gewinne" in
-- der Konten-Übersicht (accountBalance.ts: gewinneGesamt = Summe aus
-- gewinn_gesamt + gewinn_spieltag). get_player_season_balances() bekommt
-- dafür eine zusätzliche Spalte; die Sichtbarkeitsregeln (nur der fertig
-- berechnete Wert, keine einzelnen Buchungszeilen) bleiben unverändert.
--
-- Der Rückgabetyp ändert sich (neue Spalte) - create or replace function
-- erlaubt das bei RETURNS TABLE nicht, daher erst drop, dann neu anlegen.

drop function if exists public.get_player_season_balances(uuid[]);

create function public.get_player_season_balances(p_season_ids uuid[])
returns table(player_id uuid, season_id uuid, gesamt_saldo numeric, gewinne numeric)
language plpgsql
security definer
...
begin
  ...
  tx_agg as (
    select
      ...
      sum(case when t.typ in ('gewinn_gesamt', 'gewinn_spieltag') then t.betrag else 0 end) as gewinne
  ...
  select
    ...
    coalesce(tx.gewinne, 0) as gewinne
  from all_players ap
